...
def fill_with_draw(plots): 
    '''Create and fill all plots using Sample.chain.Draw
       plot.attributes, plot.weight, and plot.selectionString can be string if common among the samples,
       if not they can be a function with the sample as argument and a string as output:
       e.g. plot.weight = lambda samp: "puWeight"
    '''
    ...
    for selectionString in selectionStrings:
        ...
        for sample in samples:
            ##DELME weight_string_ = sample.combineWithSampleWeight( weight_string )
            logger.info( "Now working on sample %s",  sample.name)
            # find all plots whose stack contains the current sample
            plots_for_sample = [p for p in plots_for_selection if sample in p.stack.samples]
            ...
            for plot in plots_for_sample:
                for index in plot.sample_indices:
                    ...
                    plot_attributes = plot.attributes if not hasattr( plot.attributes[0], '__call__') else [plot.attributes[0](sample)]
                    #print(plot_attributes)
                    if len(plot_attributes)>1:  
                        raise NotImplementedError( "So far can only do 1D plots with fill_with_draw.")
                    if type(plot_attributes[0])!=str:
                        raise NotImplementedError( "Please provide a string to draw. Got %r." % plot_attributes[0] ) 

                    # get selecton string for the sample
                    weight_string = plot.weight if not hasattr( plot.weight, '__call__') else plot.weight(sample)
                    #print ('weight_string, before' , weight_string)
                    weight_string = sample.combineWithSampleWeight( weight_string )    
                    #print ('weight_string, after', weight_string)
                    logger.info( "For plot %s, weight string: ", plot.name, weight_string )

                    # get weight string for the sample
                    selection_string = plot.selectionString if not hasattr( plot.selectionString, '__call__') else plot.selectionString(sample)
                    #print ('selection_string, before' , selection_string)
                    selection_string = sample.combineWithSampleSelection( selection_string )
                    #print ('selection_string, after' , selection_string)
                    logger.info( "For plot %s, selection string: ", plot.name , selection_string )

                    args = ( plot_attributes[0]+">>"+plot.histos[index[0]][index[1]].GetName(), "(%s)*((%s)>0)"%(weight_string, selection_string), 'goff')  # which is better (%s)>0 or (%s)!=0 ?
                    setattr( plot.histos[index[0]][index[1]], "sample_name", plot.stack[index[0]][index[1]].name ) # attaching the sample name to the histo
                    logger.debug( "Draw arguments: %s", ", ".join(args) )

                    sample.chain.Draw( *args )

                    logger.info( "Sample %s plot %s has integral %3.2f", sample.name, plot.name, plot.histos[index[0]][index[1]].Integral() )
...
def draw(plot, \
    ...
    junks = {'junks':[]} ## keep objects here to keep plots
...
#        legend_.SetBorderSize(1)
        for l in histos:
            for h in l:
                ...
                legend_option = getattr(h, "legendOption", "lpf")
                legend_.AddEntry(h, legend_text, legend_option)
        legend_.Draw()
        junks['legends'].append(legend_)

    for o in drawObjects:
        if o:
            if type(o) in [ ROOT.TF1 ]:
                o_drawOption = getattr(o, 'drawOption', 'same')
                o.Draw( o_drawOption )
            else:
                o_drawOption = getattr(o, 'drawOption', '')
                o.Draw( o_drawOption )
        else:
    ...
    if ratio is not None:
        ...
        for o in ratio['drawObjects']:
            if o:
                o.Draw(getattr(o,'drawOption',""))
                junks['junks'].append(o)
            else:
